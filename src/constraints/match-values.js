...
export default function constraintMatchValues(values, options) {
  ...
  const targetValuesIn = store.match(
    constraintUri,
    FORM("valuesIn"),
    undefined
  );
  const targetValuesNotIn = store.match(
    constraintUri,
    FORM("valuesNotIn"),
    undefined
  );
  ...
  if (values.length == 0) {
    ...
    const elements = targetValuesIn[0].object.elements.map(
      (element) => element.value
    );
    return values.filter((value) => elements.includes(value.value)).length > 0;
  } else if (targetValuesNotIn.length) {
    const elements = targetValuesNotIn[0].object.elements.map(
      (element) => element.value
    );
    return values.filter((value) => elements.includes(value.value)).length == 0;
  } else return false;
