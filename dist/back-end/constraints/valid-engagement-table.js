...
function constraintValidEngagementTable(table, options) {
  ...
  engagementEntries.map(e => e.object).forEach(entry => {
    ...
    const hasRequiredTriples = hasTriples(entry, [existingStaffPredicate, additionalStaffPredicate, volunteersPredicate], store, sourceGraph);

    if (!hasRequiredTriples) {
      isValidTable = false;
    } else {
      // Each entry should have a valid and positive ints as numbers
      const validPositiveInts = hasPositiveNumbers(entry, [existingStaffPredicate, additionalStaffPredicate, volunteersPredicate], store, sourceGraph);
...
function hasPositiveNumbers(entry, predicates, store, sourceGraph) {
  ...
  predicates.forEach(predicate => {
    ...
    result = result && (0, _positiveNumber.default)(value);
  });
  return result;
}